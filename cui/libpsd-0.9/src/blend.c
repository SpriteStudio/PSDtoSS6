...
{
	psd_argb_color * dst_data, * src_data, dst_color, src_color , *org_dst , *dstlookup , *dsttemp;
	int addr_temp;
...
#if 0
				/*kurooka クリッピングマスクの処理*/

				if (layer->clipping == 1 )
				{
					//グループとして設定されている画素の情報を取得
					addr_temp = dst_data - org_dst;
					dsttemp = layer->group_layer->image_data + addr_temp;
					dst_alpha = PSD_GET_ALPHA_COMPONENT(*dsttemp);
					if ( dst_alpha == 0 ) continue;

				}
#endif
				/*kurooka クリッピングマスクの処理 ここまで*/
